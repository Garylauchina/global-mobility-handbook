...
- Select a bounded, reproducible set that covers different route types, regions, evidence grades, due and not-yet-due pages, active and warning states, dynamic formulas, and known recent-change cases.
...
- Adjudicate disagreements against the underlying official sources. If a core field cannot be positively re-established, use `evidence-incomplete` and do not advance the central page review record.
...
- Record the official announcement or publication date, effective date, old rule, new rule, transitional treatment, official source and event evidence grade when available. Site review dates belong in central records or Git, following the [update policy](../../../../UPDATE_POLICY.md). Use `未公开` or `不适用` instead of inventing a date or transition.
