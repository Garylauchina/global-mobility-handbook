...
- Keep each policy record on its category/country content page or on a category-defined route leaf; programs reviewed together may use separate blocks on that page. Do not add a global comparison table, ranking, score, visa-free count, success rate, or personalized recommendation.
...
Apply the status and standard warning rules in `UPDATE_POLICY.md`. Keep source evidence and uncertainty visible. Policy pages must not contain site verification dates or per-page review cadence; central records live in `.maintenance/review-state.json`, and only the project homepages present the full-library review baseline. Preserve official publication, effective and deadline dates.
...
Do not advance review records or the homepage baseline for editorial work, publication or automated scans. Record material closures, reopenings, threshold changes, and category changes in `CHANGELOG.md`.
...
- Update central review paths and every affected category or country index when adding, removing, or moving a policy content page.
...
- Select checks by impact using `MAINTENANCE.md`; publication requires the full set, a strict build into an external absolute temporary directory, and Chinese/English search validation. For instruction-only edits, validate changed Skills and links.
